...
class CondConf:
    ...
    def setup_problem_precomputed(
        ...
        """
        setup_problem_precomputed sets up the final fitting problem for a
        particular calibration set when phi, scores are precomputed
        ...
        phi_calib : np.ndarray
            phi(x) for the calibration set

        scores_calib : np.ndarray
            Conformity scores for the calibration set
        """
        self.x_calib = x_calib
        self.phi_calib = phi_calib
...
# We assume ProbAccum is imported from the Romano code:
# from your_romano_module import ProbAccum
...
def romano_score_vector(old_model, x, eps=None):
    ...
    """
    x = np.asarray(x)
    x = x.reshape(1, -1)  # force shape (1, d)
...
def compute_trust_scores(train_features, train_labels, new_features, new_preds):
    ...
    new_d = np.tile(None, (new_features.shape[0], n_labels))
    for label_idx in range(n_labels):
        new_d[:, label_idx] = index_arr[label_idx].search(new_features, 2)[0][:, -1]

    # Compute trust scores for validation set
    sorted_d = np.sort(new_d, axis=1)
    d_to_pred = new_d[range(new_d.shape[0]), new_preds]
    d_to_closest_not_pred = np.where(
        sorted_d[:, 0] != d_to_pred, sorted_d[:, 0], sorted_d[:, 1]
    )
    min_dist = 1e-12
    trustscores = d_to_closest_not_pred / (d_to_pred + min_dist)

    return trustscores

def build_phi(conf_scores, trust_scores, conf_bins, trust_bins,
              num_bins_conf, num_bins_trust):
    n = conf_scores.shape[0]

    conf_phi  = np.zeros((n, num_bins_conf))
    trust_phi = np.zeros((n, num_bins_trust))

    for b in range(num_bins_conf):
        mask = (conf_scores <= conf_bins[b+1]) & (conf_scores > conf_bins[b])
        conf_phi[:, b] = mask.astype(float)

    for b in range(num_bins_trust):
        mask = (trust_scores <= trust_bins[b+1]) & (trust_scores > trust_bins[b])
        trust_phi[:, b] = mask.astype(float)

    # Drop last bin in each group → remove deterministic “sum to 1” relation
    psi = np.concatenate(
        [conf_phi[:, :-1], trust_phi[:, :-1]],
        axis=1
    )  # shape: (n, (B_conf - 1) + (B_trust - 1))

    return psi


def get_bin_edges(confidence_scores, trust_scores, num_bins, bin_strategy):
...
class CondConf_trustscore_modified:
    def __init__(
            self,
            score_fn : Callable,
            Phi_fn : Callable,
    ...
    def setup_problem(
        ...
        self.cvx_problem = _setup_cvx_problem_ts(
            self.x_calib,
            self.scores_calib,
            self.phi_calib,
            self.infinite_params
    ...
    def predict(
        ...
        if exact:
            ...
            _solve = partial(_solve_dual_ts, gcc=self, x_test=x_test, quantile=quantile, threshold=threshold)

            if S_min is None:
                S_min = np.min(self.scores_calib)
            if S_max is None:
                S_max = np.max(self.scores_calib)
            lower, upper = _binary_search_ts(_solve, S_min, S_max * 2)
    ...
    def predict_input_adjusted(
        ...
        x_test : np.ndarray
            Single test point
        phi_test : np.ndarray
            Single point phi(x_test)
        scores_test : np.ndarray
            Conformity scores for the test set
        S_min : float = None
            Lower bound (if available) on the conformity scores
        S_max : float = None
            Upper bound (if available) on the conformity scores
        ...
        prediction_set = None
        if quantile > 0.5:
            prediction_set = (scores_test <= score_cutoff)
        else:
            prediction_set = (scores_test > score_cutoff)
    ...
    def estimate_coverage(
        ...
        """
        weights = weights.reshape(-1,1)
        prob = _setup_cvx_problem_calib_ts(
            quantile,
            self.x_calib,
            self.scores_calib,
            self.phi_calib,
    ...
    def verify_coverage_classification(
            self,
            phi_test : np.ndarray,
            scores_test : np.ndarray,
            quantile : float,
        ...
        ---------
        phi_test : np.ndarray
            A vector of test phi(x)
        scores_test : np.ndarray
            A vector of test conformity scores
        quantile : float
            Nominal quantile level
        ...
        if self.infinite_params.get('kernel', FUNCTION_DEFAULTS['kernel']):
            raise ValueError("Function verify_coverage_classification doesn't support RKHS quantile regression for now.")
        else:
            for phi_val, score_val in tqdm(zip(phi_test, scores_test), total=len(phi_test)):
                S_true = score_val
                naive_duals, naive_primals = self._get_calibration_solution(
                    quantile
                )
...
def _binary_search_ts(func, min, max, tol=1e-3):
    min, max = float(min), float(max)
    assert (max + tol) > max
    while (max - min) > tol:
        mid = (min + max) / 2
...
def _solve_dual_ts(S, gcc, x_test, quantile, threshold=None):
    prob = _finish_dual_setup_ts(
        gcc.cvx_problem,
        S,
        x_test,
...
def _setup_cvx_problem_ts(
    ...
    if kernel is None: # no RKHS fitting
        ...
        _, L_11 = _get_kernel_matrix_ts(x_calib, kernel, gamma)
...
def _get_kernel_matrix_ts(x_calib, kernel, gamma):
    K = pairwise_kernels(
        X=x_calib,
        metric=kernel,
        gamma=gamma
...
def _setup_cvx_problem_calib_ts(
    quantile,
    x_calib,
    scores_calib,
    phi_calib,
    ...
    if kernel is None: # no RKHS fitting
        ...
        _, L = _get_kernel_matrix_ts(x_calib, kernel, gamma)
